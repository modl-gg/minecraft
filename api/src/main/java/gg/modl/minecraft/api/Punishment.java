...
public class Punishment {
    ...
    public String getReason() {
        // First try to get reason from data map
        Object reasonObj = getDataMap().get("reason");
        if (reasonObj instanceof String && !((String) reasonObj).isEmpty()) {
            return (String) reasonObj;
        }

        // Fall back to notes, skipping auto-generated ones
        List<Note> noteList = getNotes();
        for (Note note : noteList) {
            String text = note.getText();
            if (text != null && !isAutoGeneratedNote(text)) {
                return text;
            }
        }
