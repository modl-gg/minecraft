...
public class NeoForgeBridgePluginContext implements BridgePluginContext {
    private final MinecraftServer server;
    private final Path dataFolder;
    private final Logger logger;
    private final NeoForgeBridgeScheduler scheduler;
    private final NeoForgePlayerProvider playerProvider;

    public NeoForgeBridgePluginContext(MinecraftServer server, Path dataFolder) {
        this.server = server;
        this.dataFolder = dataFolder;
        this.logger = Logger.getLogger("modl");
        this.scheduler = new NeoForgeBridgeScheduler(server);
        this.playerProvider = new NeoForgePlayerProvider(server);
    }

    @Override public BridgeScheduler getScheduler() { return scheduler; }
    @Override public BridgePlayerProvider getPlayerProvider() { return playerProvider; }
    ...
    @Override
    public String getMinecraftVersion() { return server.getServerVersion(); }

    public NeoForgeBridgeScheduler getBridgeScheduler() { return scheduler; }
}
